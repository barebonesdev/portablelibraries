...
namespace HtmlAgilityPack
{
	...
	public partial class HtmlNode
	{
        ...
        public static readonly string HtmlNodeTypeNameCDATA = "#cdata";
		...
		static HtmlNode()
		{
            ...
            //ADDED: RSS files use the "link" element, so I'll allow it
            //otherwise it's typically used like <link ref="stylesheet">
		...
		public HtmlNode(HtmlNodeType type, HtmlDocument ownerdocument, int index)
		{
			...
			if ((-1 != index) || (type == HtmlNodeType.Comment) || (type == HtmlNodeType.Text) || type == HtmlNodeType.CDATA) return;
		...
		public virtual string InnerText
		{
			get
			{
                ...
                if (_nodetype == HtmlNodeType.CDATA)
                    return ((HtmlCDATANode)this).Data;
        ...
        /// <summary>
        /// Gets the first node of the given type that immediately precedes this node.
        /// </summary>
        /// <param name="ofType"></param>
        /// <returns></returns>
        public HtmlNode PreviousSiblingOfType(HtmlNodeType ofType)
        {
            HtmlNode n = PreviousSibling;

            while (n != null && n.NodeType != ofType)
                n = n.PreviousSibling;
        ...
        public string Grab(HtmlInstruction instruction)
        {
            HtmlNode node;

            if (instruction.XPath == null)
                node = this;
            else
                node = this.SelectSingleNode(instruction.XPath);
            

            if (node == null)
                return null;

            if (instruction.SelectAttribute != null)
            {
                return node.GetAttributeValue(instruction.SelectAttribute, null);
            }
		...
		public HtmlNode CloneNode(bool deep)
		{
			...
			switch (_nodetype)
			{
                ...
                case HtmlNodeType.CDATA:
                    ((HtmlCDATANode)node).Data = ((HtmlCDATANode)this).Data;
                    return node;
			}
		...
		/// <summary>
		/// Gets all Descendant nodes in enumerated list. A child is followed by its descendants before being followed by the child's sibling (thus in-order of HTML document).
		/// </summary>
		/// <returns></returns>
		...
		public void WriteTo(XmlWriter writer)
		{
			switch (_nodetype)
			{
                ...
                case HtmlNodeType.CDATA:
                    string data = ((HtmlCDATANode)this).Data;
        ...
        private IEnumerable<HtmlNode> selectNodes(string xpath)
        {
            XPathNavigator navigator = new XPathNavigator(xpath);

            XPathNavigator.Instruction instr;

            IEnumerable<HtmlNode> nodes = null;


            instr = navigator.NextInstruction();
            ...
            switch (instr.Type)
            {
                case XPathNavigator.InstructionType.SelectCurrent:
                    nodes = new HtmlNode[] { this };
                    break;

                case XPathNavigator.InstructionType.SelectParent:

                    if (ParentNode == null)
                        return new HtmlNode[0];

                    nodes = new HtmlNode[] { ParentNode };
                    break;

                default:
                    nodes = new HtmlNode[] { OwnerDocument.DocumentNode };
                    break;
            }
            ...
            do
            {
                ...
                switch (instr.Type)
                {
                    case XPathNavigator.InstructionType.SelectChildren:
                        nodes = nodes.SelectMany(i => i.ChildNodes);
                        break;

                    case XPathNavigator.InstructionType.SelectDescendants:
                        nodes = nodes.SelectMany(i => i.Descendants());
                        break;

                    case XPathNavigator.InstructionType.MatchName:

                        if (name.Equals("*"))
                            continue;

                        nodes = nodes.Where(i => i.Name.Equals(name, StringComparison.CurrentCultureIgnoreCase));
                        break;

                    case XPathNavigator.InstructionType.MatchAttribute:
                        ...
                        {
                            if (value != null)
                                nodes = nodes.Where(i => hasAnyAttributeValue(i, value));
                        }

                        else
                        {
                            if (value != null)
                                nodes = nodes.Where(i => hasAttributeAndValue(i, name, value));

                            else
                                nodes = nodes.Where(i => i.Attributes.Contains(name));
                        }

                        break;

                    case XPathNavigator.InstructionType.SelectCurrent:
                        //nothing, it keeps current selection
                        break;

                    case XPathNavigator.InstructionType.SelectParent:
                        nodes = nodes.Select(i => i.ParentNode).Where(i => i.ParentNode != null);
                        break;
                }

            } while ((instr = navigator.NextInstruction()) != null);


            return nodes;
        }

        private static bool hasAttributeAndValue(HtmlNode node, string name, string value)
        {
            string actualValue = node.GetAttributeValue(name, null);
        ...
        private static bool hasAnyAttributeValue(HtmlNode node, string value)
        {
        ...
        public bool HasAttribute(string attributeName, string attributeValue)
        {
            HtmlAttribute a = Attributes[attributeName];

            if (a == null)
                return false;

            return a.Value.Equals(attributeValue);
        }
        ...
        public string[] Classes
        {
            get
            {
                ...
                string value = Attributes["class"].Value;
        ...
        public bool HasClass(string c)
        {
            ...
            for (int i = 0; i < classes.Length; i++)
                if (classes[i].Equals(c, StringComparison.CurrentCultureIgnoreCase))
                    return true;
