	...
	public partial class HtmlDocument
	{
		...
		internal HtmlNode CreateNode(HtmlNodeType type, int index)
		{
			switch (type)
			{
                ...
                case HtmlNodeType.CDATA:
                    return new HtmlCDATANode(this, index);
		...
		private bool NewCheck()
		{
			...
			if ((_index - 1) <= (Text.Length - 2))
			{
				if (Text[_index] == '!')
				{
                    if (startsWith(Text, "[CDATA[", _index + 1))
                    {
                        PushNodeStart(HtmlNodeType.CDATA, _index + 8); // length of "![CDATA["
                        _state = ParseState.CDATA;
                        return true;
                    }
        ...
        private static bool startsWith(string str, string other, int startIndex)
        {
            for (int i = 0; i < other.Length; i++, startIndex++)
            {
                if (startIndex >= str.Length)
                    return false;

                if (other[i] != str[startIndex])
                    return false;
            }
		...
		private bool PushNodeEnd(int index, bool close)
		{
			...
			if ((_currentnode._nodetype == HtmlNodeType.Text) ||
                ...
                _currentnode._nodetype == HtmlNodeType.CDATA)
			{
